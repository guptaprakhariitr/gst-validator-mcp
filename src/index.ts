...
function renderLanding(env: Env, url: URL): string {
  const productName = env.PRODUCT_NAME ?? "gst-validator-mcp";
  const tagline = env.PRODUCT_TAGLINE ?? "Validate Indian GSTIN numbers for AI agents — structure, state code, embedded PAN, and GSTN mod-36 check character.";
